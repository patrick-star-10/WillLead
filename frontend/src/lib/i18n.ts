...
const messages = {
  en: {
    ...
    intentPickerNote:
      'Choose which wallet intent to inspect. The transfer intent stays editable, while the swap demo intent reflects the live deployed route.',
    transferIntentLabel: 'Transfer Intent',
    swapIntentLabel: 'Swap -> lREACT Intent',
    transferPlanKicker: 'Transfer Intent',
    transferPlanNote: 'Define the transfer intent this wallet should keep executing.',
    planRouteNote:
    ...
    swapIntentNote:
      'This demo intent listens across all Uniswap v3 Sepolia WETH/Circle USDC pools and reacts by requesting lREACT through the Reactive faucet route.',
    ...
    swapIntentSaveNote:
      'Saving writes the recipient, funding amount per trigger, and max executions into the onchain faucet intent. After that, matching swaps can trigger it automatically.',
    swapIntentFundingNote:
      'This swap intent needs its own callback proxy credit. Funding the wallet intent does not fund the faucet execution contract.',
    ...
    swapRuntimeNote:
      'Watch the runtime route for the upstream Uniswap swap intent. Manual test-signal controls are intentionally removed here.',
    ...
    submittingSwapIntentTransaction: 'Saving swap intent to the faucet execution contract...',
  ...
  'zh-CN': {
    ...
    intentPickerNote: '选择你现在要查看的 intent。转账 intent 仍然可编辑，swap demo intent 则展示当前已部署的真实路由。',
    transferIntentLabel: '转账 Intent',
    swapIntentLabel: 'Swap -> lREACT Intent',
    transferPlanKicker: '转账 Intent',
    transferPlanNote: '定义这个钱包需要持续执行的转账 intent。',
    ...
    resumePlan: '恢复计划',
    swapIntentKicker: 'Swap Intent',
    swapIntentNote: '这条 demo intent 会监听 Uniswap Sepolia 上 WETH/Circle USDC 的全部 v3 池子，并通过 Reactive faucet 路由去请求 lREACT。',
    ...
    swapIntentSaveNote: '保存后会把 recipient、每次触发金额和最大执行次数写进链上的 faucet intent。后续只要有符合条件的 swap，这条路由就会自动响应。',
    swapIntentFundingNote: '这条 swap intent 需要单独的 callback proxy 额度。给 wallet intent 充值，并不会同时给这个 faucet 执行合约充值。',
    ...
    swapRuntimeNote: '查看上游 Uniswap swap intent 的 runtime route。这里会刻意移除手动测试信号按钮。',
    ...
    submittingSwapIntentTransaction: '正在把 swap intent 写入 faucet 执行合约...',
