...
const messages = {
  en: {
    ...
    heroSubcopy:
      'Save one intent, wait for external events, and show the full Origin -> Reactive -> Destination proof trail.',
    ...
    utilityActionsNote:
      'Keep these controls nearby for demos, but out of the main storytelling path.',
    ...
    advancedRouteNote:
      'These fields prove the wallet can be rebound to another deployed Reactive route. Keep them collapsed during demos unless route configurability is the point.',
    planRouteNote:
    ...
    runtimeReadinessNote:
      'These three checks are the fastest way to tell whether the next external event can complete end-to-end.',
  ...
  'zh-CN': {
    ...
    heroSubcopy: '先保存一次 intent，再等待外部事件触发，并把 Origin -> Reactive -> Destination 的完整 proof 链路展示出来。',
    ...
    utilityActions: '辅助操作',
    utilityActionsNote: '这些操作适合放在 demo 附近备用，但不应该抢主叙事的注意力。',
    ...
    advancedRouteNote:
      '这些字段用于证明这只钱包可以绑定到另一套已部署的 Reactive route。黑客松 demo 时建议默认收起，只有在需要强调可配置性时再展开。',
    ...
    runtimeReadinessNote:
      '下面这三个检查项，是判断下一次外部事件能不能完整跑通的最快方式。',
